...
export function generateProject(state: BuilderState): GenFile[] {
  ...
  if (state.auth.strategy !== 'none') {
    if (state.auth.register) routeLines.push("$router->add('POST', '" + prefix + "/auth/register', fn($req) => (new AuthController())->register($req, $config));");
    if (state.auth.login) routeLines.push("$router->add('POST', '" + prefix + "/auth/login', fn($req) => (new AuthController())->login($req, $config));");
    routeLines.push("$router->add('POST', '" + prefix + "/auth/logout', fn($req) => (new AuthController())->logout($req));");
    routeLines.push("$router->add('POST', '" + prefix + "/auth/refresh', fn($req) => (new AuthController())->refresh($req, $config));");
    routeLines.push("$router->add('GET', '" + prefix + "/auth/me', function ($req) use ($config) { requireAuth($req, $config); (new AuthController())->me($req); });");
  }
  ...
  for (const t of sortedTables) {
    ...
    routeLines.push("$router->add('GET', '" + prefix + '/' + t.name + "', fn($req) => (new " + c + '())->list($req));');
    routeLines.push("$router->add('POST', '" + prefix + '/' + t.name + "', fn($req) => (new " + c + '())->create($req));');
    routeLines.push("$router->add('GET', '" + prefix + '/' + t.name + "/{id}', fn($req, $p) => (new " + c + "())->read($req, $p['id']));");
    routeLines.push("$router->add('PUT', '" + prefix + '/' + t.name + "/{id}', fn($req, $p) => (new " + c + "())->update($req, $p['id']));");
    routeLines.push("$router->add('PATCH', '" + prefix + '/' + t.name + "/{id}', fn($req, $p) => (new " + c + "())->patch($req, $p['id']));");
    routeLines.push("$router->add('DELETE', '" + prefix + '/' + t.name + "/{id}', fn($req, $p) => (new " + c + "())->delete($req, $p['id']));");
  }
  ...
  const refreshTokensSql = state.auth.strategy !== 'none'
  ...
  const clientAuthFns = state.auth.strategy !== 'none' ? [
  ...
  if (state.auth.strategy !== 'none' && state.auth.generateFrontend) {
    files.push({
  ...
  files.push({
    ...
    content: header('Generated API tests — health, auth, CRUD, validation, pagination') +
      ...
      (state.auth.strategy !== 'none' ? "    public function testAuthRoutesRegistered(): void { $src = file_get_contents(__DIR__ . '/../backend/public/index.php'); foreach (['/auth/register','/auth/login','/auth/logout','/auth/refresh','/auth/me'] as $r) { $this->assertStringContainsString($r, $src, 'missing ' . $r); } }\n" : '') +
